...
class DivCo2Model(BaseModel):
    ...
    def forward(self):
        ...
        self.fake_B_random = self.netG(self.real_A_encoded.repeat(
                                 [self.opt.num_negative+2]+list(np.ones(len(self.real_A_encoded.shape[1:]), dtype=np.uint8)) ), self.z_random)
        ...
        if self.opt.lambda_z > 0.0 or self.opt.lambda_contra > 0.0:
            ...
            self.mu2 = nn.AdaptiveAvgPool2d(1)(self.perceptualnet(self.fake_B_random)).view(self.fake_B_random.shape[0], -1)  # mu2 is a point estimate
